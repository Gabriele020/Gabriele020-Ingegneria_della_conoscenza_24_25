...
def getUserMovie(choice):
    title = input("Immetti il nome del film o serie TV che ti e' piaciuto: ").lower()
    typeM = input(title + ' è un film? (s/n) \n -> ').lower()
    while (str(typeM) != 's' and str(typeM) != 'n'):
        typeM = input("Immetti un'opzione valida --> ")
    if typeM == 's':
        typeM = 'Movie'
    else:
        typeM = 'TV Show'
    country = input('immetti il paese di produzione: \n -> ').lower()
    yr = input ('Immetti l`anno di rilascio: \n -> ').lower()
    yr=releaseYear(yr)
    cast = input('Immetti un membro del cast: \n -> ').lower()
    rating = input('Dai un voto da 1 a 10 sul film/serie TV: \n -> ').lower()
    genreM=''
    if choice == 1:
